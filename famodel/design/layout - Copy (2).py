...
class Layout(Project):
    ...
    def generateGridPoints(self, Xu, trans_mode, boundary_index=-1):
        ...
        '''  The next lines check each point 3X and are a bit complex
        ...
        '''  # The alternative below is simpler to look at and only does a bound check once per point
    ...
    def pareGridPoints(self,points_ibe):
        ...
        #for i in range (0, n_ibe): # if i in points_sorted_idx[:nt]:
        #    if i in points_sorted_idx[:nt]:
        for i in points_sorted_idx[:nt]:    
                pared_points.append(points_ibe[i])
                pared_turb_rot.append(self.turb_rot[i])
                pared_grid_positions.append(self.grid_positions[i])
        ...
        # pull out the nt points that are furthest from the bounds
        furthest_points = list([points_ibe[i] for i in range (0, n_ibe) if i in points_sorted_idx[:nt]]) 
        alt = list([points_ibe[i] for i in points_sorted_idx[:nt]])
        
        self.grid_positions = list(self.grid_positions[i] for i in range (0, n_ibe) if i in points_sorted_idx[:nt])
        
        if self.alternate_rows:
            furthest_rotations = list(self.turb_rot[i] for i in range (0, n_ibe) if i in points_sorted_idx[:nt]) 
        
        
        for j in range(nt):
            print([furthest_points[j], self.grid_positions[j]])
        
        for j in range(nt):
            print([pared_points[j], pared_grid_positions[j]])
        ...
        if len(points_sorted_idx)< nt:
            ...
            n_leftover = nt-len(points_sorted_idx)
            
            # choose point outside bounds for leftovers
            leftover_loc = Point(min_x-1,min_y-1)
            furthest_points.extend([leftover_loc]*n_leftover)
            
            # add dummy grid positions for points out of bounds
            for i in range(n_leftover): self.grid_positions.append([0,0]) 
            
            if self.alternate_rows:
                furthest_rotations.extend([0]*n_leftover)
        ...
        for i in range(self.nt):
            self.turb_coords[i,0] = furthest_points[i].x
            self.turb_coords[i,1] = furthest_points[i].y
